...
case "${1:-}" in
  ...
  --help|-h)
    ...
    echo "  DECK_PLUGIN_DIR Plugin path on Deck (default: ~/homebrew/plugins/deck-controller)"
    exit 0
    ;;
esac
...
if ! ssh $SSH_OPTS "${DECK_USER}@${DECK_HOST}" echo "ok" &>/dev/null; then
...
# shellcheck disable=SC2086
ssh $SSH_OPTS "${DECK_USER}@${DECK_HOST}" "mkdir -p ${DECK_DEPLOY_TMP}/{backend,defaults,assets,dist}"
...
sync_backend() {
  ...
  rsync -az --delete \
    ...
    "$DST/assets/"
  # shellcheck disable=SC2086
  ssh $SSH_OPTS "${DECK_USER}@${DECK_HOST}" "sudo -n /usr/bin/cp -rf ${DECK_DEPLOY_TMP}/* ${DECK_PLUGIN_DIR}/"
...
sync_frontend() {
  ...
  # shellcheck disable=SC2086
  ssh $SSH_OPTS "${DECK_USER}@${DECK_HOST}" "sudo -n /usr/bin/cp -rf ${DECK_DEPLOY_TMP}/dist/* ${DECK_PLUGIN_DIR}/dist/"
...
restart_plugin() {
  ...
  # shellcheck disable=SC2086
  ssh $SSH_OPTS "${DECK_USER}@${DECK_HOST}" "sudo systemctl restart plugin_loader" 2>/dev/null || {
